...
import { get, post, put, errorRate } from '../src/helpers/http.js';
...
export default function (data) {
  ...
  group('Users API', () => {
    group('List users', () => {
      const res = get(`${BASE_URL}/api/users?page=1`);
      check(res, {
    ...
    group('Get single user', () => {
      const res = get(`${BASE_URL}/api/users/2`);
      check(res, {
        'status is 200':    r => r.status === 200,
  ...
  group('Create & Update', () => {
    ...
    group('Update user', () => {
      const res = put(`${BASE_URL}/api/users/2`, { job: 'Senior QA Lead' });
      check(res, {
        'status is 200':  r => r.status === 200,
